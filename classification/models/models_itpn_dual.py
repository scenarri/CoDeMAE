...
class iTPN_dual(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, mlp_depth=3, depth=24,
        ...
        #     nn.Sequential(norm_layer(fpn_dim), nn.Linear(fpn_dim, embed_dim, bias=True)))
        # if self.num_outs >= 2:
        #     self.decoder_embed.append(
        #         nn.Sequential(norm_layer(fpn_dim), nn.Linear(fpn_dim, embed_dim // 4, bias=True)))
        # if self.num_outs >= 3:
        #     self.decoder_embed.append(
        #         nn.Sequential(norm_layer(fpn_dim), nn.Linear(fpn_dim, embed_dim // 16, bias=True)))
    ...
    def forward_features(self, x, ids_keep=None, mask=None):
        ...
        if self.modality == 'RGB':
            ...
            x_sar = self.patch_embed_sar(x_sar)
            x = torch.cat([x_rgb, x_sar], dim=0)
            Hp, Wp = H // self.patch_embed_rgb.patch_size[0], W // self.patch_embed_rgb.patch_size[1]
        else:
