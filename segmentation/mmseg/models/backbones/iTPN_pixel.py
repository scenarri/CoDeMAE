...
class iTPN_pixel(nn.Module):
    def __init__(self, img_size=512, patch_size=16, in_chans=3, num_classes=80, embed_dim=512, mlp_depth1=3,
                 mlp_depth2=3, depth=24, fpn_dim=256, num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4., fpn_depth=1,
                 qkv_bias=True, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.0,
                 norm_layer=nn.LayerNorm, ape=True, rpe=True, patch_norm=True, use_checkpoint=True,
                 num_outs=4, init_cfg=None, **kwargs):
        super().__init__()
...
if __name__ == '__main__':
    model = iTPN_pixel(img_size=224,
                       ...
                       init_cfg=dict(type='Pretrained', checkpoint='../weights/itpn_base_fpn256.pth'))
    # # model.init_weights()
    # model.eval()
    ...
    import torch
    ckpt = torch.load(r'G:\project\iTPN-main\weights\INP\base_clip.pth')
    print()