...
class SatViT_Model(nn.Module):
    def __init__(self, modality='RGB', num_classes=19):
        super().__init__()
        self.modality = modality
        
        self.backbone = SatViT()
        checkpoint = torch.load(r'G:\project\mycls\weights/SatViT/SatViT_V1.pt', map_location="cpu")#['mae_model']
    ...
    def forward(self, s2, s1):
        ...
        data = torch.cat([s2, s1], dim=1)
        data_ins = rearrange(data, 'b c (h i) (w j) -> b (h w) (c i j)', i=16, j=16)
        
        x = self.backbone.linear_input(data_ins) + self.backbone.pos_embed
        encodings = self.backbone.encoder(x)
        encodings = encodings.mean(dim=1)
        
        out = self.head(encodings)
        return out
