...
class PretrainedCROMA(nn.Module):
    def __init__(self, pretrained_path='./weights/CROMA/CROMA_base.pt', size='base', modality='Both',
                 image_resolution=120, num_classes=10):
        """
        ...
        assert modality in ['Both', 'RGB', 'SAR'], f'modality must be either all, s1, or s2, not {modality}'
        ...
        if modality == 'SAR':
            ...
            self.head = torch.nn.Linear(self.encoder_dim, num_classes)
            ...
            # self.head = torch.nn.Linear(3*self.encoder_dim, num_class)
            
            self.fuse = torch.nn.Linear(3 * self.encoder_dim, self.encoder_dim)
            self.head = torch.nn.Linear(self.encoder_dim, num_classes)
    ...
    def forward(self, optical_images=None, SAR_images=None):
        if self.modality == 'SAR':
            ...
            SAR_encodings = self.s1_encoder(imgs=SAR_images, attn_bias=self.attn_bias.to(
                SAR_images.device))  # (bsz, num_patches, encoder_dim)
            SAR_GAP = self.GAP_FFN_s1(SAR_encodings.mean(dim=1))  # (bsz, encoder_dim)
            
            optical_encodings = self.s2_encoder(imgs=optical_images, attn_bias=self.attn_bias.to(
                optical_images.device))  # (bsz, num_patches, encoder_dim)
            optical_GAP = self.GAP_FFN_s2(optical_encodings.mean(dim=1))  # (bsz, encoder_dim)
            
            joint_encodings = self.cross_encoder(x=SAR_encodings, context=optical_encodings,
                                                 relative_position_bias=self.attn_bias.to(
                                                     optical_images.device))  # (bsz, num_patches, encoder_dim)
            joint_GAP = joint_encodings.mean(dim=1)  # (bsz, encoder_dim)
            # feat = SAR_GAP + optical_GAP + joint_GAP
            
            feat = torch.cat((SAR_GAP, optical_GAP, joint_GAP), -1)
            feat = self.fuse(feat)
        out = self.head(feat)
        return out
