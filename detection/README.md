...
We evaluate object detection performance on DIOR (RGB) and SARDet-100K (SAR). Following established evaluation protocols, we adopt Faster RCNN as the detector. Training follows a 1$\times$ schedule (12 epochs) for DIOR and a 3$\times$ schedule (36 epochs) for SARDet-100K.

This is done based on [MMDetection](https://github.com/open-mmlab/mmdetection).
...
```
> **1. MMDet Tutorial**: See [MMDet_README.md](MMDet_README.md)
